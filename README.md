...
Сезонная иконка PWA: при установке на домашний экран берётся иконка текущего сезона (`icon-192-<season>.png`).

### Обновление иконки PWA

- **Android/Chrome** — иконка обновляется автоматически при обновлении манифеста.
- **iOS/Safari** — иконка закрепляется в момент добавления на экран «Домой» и **не обновляется автоматически** (ограничение Apple). Чтобы увидеть новую иконку, нужно удалить приложение с домашнего экрана и добавить заново. При повторном добавлении свежая иконка гарантирована версионированием URL (`?v=N` в `apple-touch-icon`, константа `ICON_VER` в `frontend/index.html`) — при каждом изменении иконок bump-те этот параметр.
