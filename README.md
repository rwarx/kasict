...
Веб-приложение расписания колледжа с автоматическим учётом замен. Работает как PWA на iPhone, Android, Windows/macOS/Linux.
...
- Белая и чёрная темы + автоматическая сезонная палитра с неон-акцентами
- День X (18 сентября каждого года): праздничный салют при открытии, метки в шапке, расписании и календаре

### Сезонность

Приложение автоматически подстраивает палитру под время года (по месяцу устройства):

| Сезон | Месяцы | Палитра |
|-------|--------|---------|
| ☀️ Лето | июнь–август | жёлто-зелёная |
| 🍂 Осень | сентябрь–ноябрь | жёлто-оранжевая |
| ❄️ Зима | декабрь–февраль | бело-голубая |
| 🌸 Весна | март–май | свежая зелёно-розовая |

Неоновые свечения применяются к кнопкам, навигации, тумблерам и чипам. Предпросмотр сезонов: `?season=winter|spring|summer|autumn`.

### День X

Каждого **18 сентября** приложение отмечает «День X»:

- при открытии запускается canvas-фейерверк (закрывается по кнопке, тапу или Esc);
- в шапке появляется чип «День X», в расписании и календаре — метки;
- салют включён всегда;
- посмотреть салют в любой день: **Настройки → Оформление → Показать салют**;
- уважается `prefers-reduced-motion` (анимация не запускается);
- для предпросмотра без ожидания даты: `?dayx=1` (принудительно показать), `?dayx=0` (скрыть).
